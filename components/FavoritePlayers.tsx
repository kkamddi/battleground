...
const favoriteStorageKey = "bgi-favorite-players";
const recentStorageKey = "bgi-recent-players";

type FavoritePlayer = { key: string; platform: PubgPlatform; nickname: string };

function readPlayers(storageKey: string, limit: number): FavoritePlayer[] {
  try {
...
export default function FavoritePlayers() {
  ...
  const [recent, setRecent] = useState<FavoritePlayer[]>([]);
  const [ready, setReady] = useState(false);

  useEffect(() => {
    const sync = () => {
      setFavorites(readPlayers(favoriteStorageKey, 10));
      setRecent(readPlayers(recentStorageKey, 8));
      setReady(true);
    };
    sync();
    window.addEventListener("storage", sync);
    window.addEventListener("bgi-favorites-change", sync);
    window.addEventListener("bgi-recent-players-change", sync);
    return () => {
      window.removeEventListener("storage", sync);
      window.removeEventListener("bgi-favorites-change", sync);
      window.removeEventListener("bgi-recent-players-change", sync);
    };
  }, []);

  function remove(playerKey: string) {
    const next = readPlayers(favoriteStorageKey, 10).filter((player) => player.key !== playerKey).map((player) => player.key);
    window.localStorage.setItem(favoriteStorageKey, JSON.stringify(next));
    window.dispatchEvent(new Event("bgi-favorites-change"));
  }

  if (!ready || (!favorites.length && !recent.length)) return null;
  ...
  return (
    <section className="favorite-players" aria-label="즐겨찾는 플레이어">
      ...
      {recent.length ? <div className={favorites.length ? "recent-player-block" : ""}>
        <div className="favorite-players-head">
          <div><span>RECENT SEARCHES</span><h2>최근 검색</h2></div>
          <p>최근 검색한 플레이어를 바로 다시 확인하세요.</p>
        </div>
        <div className="favorite-player-list recent-player-list">
          {recent.map((player) => (
          <article key={player.key}>
            <Link href={`/player/${player.platform}/${encodeURIComponent(player.nickname)}`}>
              <span className={`platform-mark ${player.platform}`}>{player.platform === "kakao" ? "K" : "S"}</span>
